...
Using the Linear connector, list issues on team `BIG` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `BIG-NNN-short-slug`, using the issue's real identifier.
...
- `Closes BIG-NNN`, using the real Linear issue identifier
